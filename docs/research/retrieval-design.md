...
| Freshness | Records the shipped 2026-07-17 change: `sync` refreshes the cache best-effort post-sweep; `search`/`sessions`/`query` still refresh before answering; `show`/`outline` stay raw-read. Cache-opening verbs wait up to 15 s on the retrieval lock (polling) before the existing failure, so the hourly sync never hard-fails an agent mid-funnel. *(Superseded in part 2026-07-18, serve-stale, Liam's verdict: when a live refresh holds the lock and a current-schema cache exists, `search`/`sessions`/`query` answer immediately from that cache read-only and print one stderr notice that results may predate the running refresh; the cache is WAL so readers coexist with the writer. The 15 s wait now applies only when nothing is servable, and its failure names the first index build instead of "retrieval is already running". `search --rebuild` keeps the wait-then-fail contract. Motivation: the 0.3.1 schema rebuild held the lock for minutes and agents read the old failure as a broken archive, issue #62.)* |
