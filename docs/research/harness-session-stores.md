...
11. **Respect configurable roots.** Claude can move all state with `CLAUDE_CONFIG_DIR`; Codex uses `CODEX_HOME`; opencode supports `OPENCODE_DB`; pi supports `--session-dir`/`PI_CODING_AGENT_SESSION_DIR`. Hard-coding only default home paths misses valid stores.
...
1. **Claude minimal restore set.** Docs strongly imply that the correctly placed main JSONL alone is enough for direct ID resume, but do not explicitly promise independence from `~/.claude.json` project state. Settle with a fresh `CLAUDE_CONFIG_DIR`, one copied transcript under the exact encoded cwd, no history/config state, and a no-prompt `--resume` discovery check.
2. **Claude in-place rewrite edge cases.** Ordinary writes appear append-oriented, but closed source leaves rename, `/compact`, `/rename`, `/branch`, crash recovery, and `/cd` implementation details unverified. Settle by recording inode, size, prefix hash, and path before and after each operation in a disposable session containing no private content.
3. **Claude lossy-slug collisions.** Docs define non-alphanumeric replacement but not conflict handling when two absolute paths encode identically. Settle with two disposable cwd paths differing only by punctuation, then inspect whether sessions share a directory and whether picker filtering separates them by in-file `cwd`.
4. **Claude retention age basis and sweep cadence.** Current docs say “older than” the cutoff at startup but do not identify mtime, birth time, last record timestamp, or a throttling interval behind `.last-cleanup`. The installed binary exposes the setting and marker strings but not enough readable implementation to establish the comparison safely. Settle with disposable files whose mtime, birth time, and last-record timestamp straddle the cutoff independently, then launch once before and once after changing `.last-cleanup`.
5. **Codex archived-only recovery without SQLite.** Source proves active-tree fallback and supported unarchive behavior, but not whether `codex unarchive <uuid>` can discover an archived file when `state_5.sqlite` is absent. Settle in a fresh `CODEX_HOME` containing only one archived rollout, then run `codex unarchive <uuid>` without sending a model prompt.
6. **Codex target machine with missing stored cwd.** Direct UUID is not cwd-filtered, but interactive resume defaults to the stored cwd. Settle by copying one non-sensitive rollout to a fresh machine/config root where that cwd does not exist, then test `codex resume <uuid> -C <existing-dir>` and `codex exec resume` without a prompt.
7. **Cursor's entire persistence contract.** Official sources do not disclose local store path, format, mutation, retention, or index behavior. Settle by installing Cursor CLI in an isolated home, creating one disposable session, observing filesystem changes, restarting, resuming by ID and folder, and checking whether logout or machine transfer changes discovery.
